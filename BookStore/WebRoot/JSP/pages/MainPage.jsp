...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css"
			href="/BookStore/css/myStyle.css">
			...
			<div class="collapse navbar-collapse" id="myNavbar">
				<ul class="nav navbar-nav">
					<li
						...
						<a data-toggle="tab"
							href="<%=basePath%>servlet/MainServlet?SelectNavBarID=0">最新上架</a>
					</li>
					<li
						class="${sessionScope.MainPage_SelectNavBarID == 1 ? 'active' : ' '}">
						<a data-toggle="tab"
							href="<%=basePath%>servlet/MainServlet?SelectNavBarID=1">为您推荐</a>
					</li>
					<li
						class="${sessionScope.MainPage_SelectNavBarID == 2 ? 'active' : ' '}">
						<a data-toggle="tab"
							href="<%=basePath%>servlet/MainServlet?SelectNavBarID=2">热卖图书</a>
					</li>
				</ul>
					...
					<li>
						<div class="navbar-form navbar-left">
							...
							<button id="searchButton" type="submit"
								class="btn btn-
	window.location.href = '/BookStore/JSP/pages/SearchPage.jsp?searchInput=' + document
			.getElement
			...
			<div class="row content">
				<div class="col-sm-2 sidenav">
					...
					<ul class="nav nav-tabs nav-stacked">
						<c:forEach items="${sessionScope.MainPage_ClassficationList}"
							var="category">
							<li
								class="${sessionScope.MainPage_SelectCategoryID == category.cID ? 'active' : ' '}">
								<a
									href="<%=basePath%>servlet/MainServlet?SelectCategoryID=${category.cID}">${category.cName}</a>
							</li>
						</c:forEach>
					</ul>
					<br>
...
 			    </div>
			...
			-->
					<c:set var="book_img_path" value="/assets/books/book_3.jpg"
						scope="page">
					</c:set>
					<c:forEach items="${sessionScope.MainPage_BookList}" var="book"
						varStatus="status">
					...
					<c:set var="currentPage"
						value="${sessionScope.MainPage_currentPage}" scope="page">
					</c:set>
					<c:set var="pageNum" value="${sessionScope.MainPage_pageNum}"
						scope="page">
					</c:set>
					<ul class="pager">
...
        			<li class="previous">
								<a
									href="<%=basePath%>servlet/MainServlet?pageNext=${currentPage - 1}">前一页</a>
							</li>
    			</c:if>
						<c:if test="${pageScope.currentPage < pageScope.pageNum}">
        			<li class="next">
								<a
									href="<%=basePath%>servlet/MainServlet?pageNext=${currentPage + 1}">后一页</a>
								</td>
								    
						</c:if>
					</ul>
